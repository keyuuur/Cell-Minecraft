...
- Visual Pass 3 candidate commit: `7257ad13abe194fcf2a875fbb9b2bd5fbbbf7bee` (`feat: complete visual pass 3 candidate`). It contains the exact locally verified app and evidence-driver source that must be used as the evidence SHA for protected Preview verification and counted Run 5. A later handoff-only checkpoint may be the Preview's tested app SHA without changing this evidence SHA.
...
- Exact committed evidence base: `7257ad13abe194fcf2a875fbb9b2bd5fbbbf7bee`. The candidate evidence and implementation were committed together after all local checks passed; only the handoff's exact-SHA reporting checkpoint remained uncommitted at this update.
...
- Pre-Preview re-gate: independent QA and Deployment/Ops reviewers found that the visual driver validated the Vercel bypass host but did not install the bypass cookie in its fresh browser context. The driver now performs the protected-Preview bootstrap itself and fails closed unless the cookie is present. This correction requires a new committed evidence SHA and fresh local calibration before counted Run 5; `7257ad13abe194fcf2a875fbb9b2bd5fbbbf7bee` remains the verified UI candidate but is superseded as the final evidence-driver SHA.
- Uncounted Calibration E completed the full post-`4eee6e0` route in 4.6 minutes, but Deployment/Ops then identified that the exact transient Run 5 payload had to remain in test memory long enough to prove backend idempotency without creating a second raw row. The live-only driver now retries that same payload once, requires the original accepted receipt fields, keeps the visible-page submission count at one, and writes no payload or ID to evidence. Calibration E is therefore superseded; one final fresh local calibration is required from the next committed harness SHA.

## 7. Current active phase and exact remaining work

Active phase: **Five-Run, Three-Pass Swarm UI Rollout - Visual Pass 3 and protected Preview Run 5**.

Remaining rollout work, in order:

1. Commit and push the verified Visual Pass 3 candidate using only explicit intended files, then record its exact SHA. Wait for green GitHub CI and a successful protected Vercel Preview built from that exact SHA; do not promote Production.
2. Run the uncounted 1024 x 680 accessibility regression against that exact protected Preview, then establish private Sheet row-count baselines.
3. Run counted Run 5 against the protected Preview at 1024 x 768, Touch Only, standard settings, using the real synthetic backend path with `isTest=true`. Verify one accepted receipt, exactly one new `RawSubmissions` test row, no `BestResults` increase, and duplicate idempotency without another raw row.
4. Inspect all original Run 5 PNGs, complete the swarm gate, generate/render/inspect the PDF, update this handoff, commit/push the reporting checkpoint, and send Email 5 exactly once.
...
- **Visual Pass 3 posture:** responsive dialog, Overview, hint, grade-breakdown, pause, Results/practice/delivery-status, and cross-screen-cohesion work passed the local candidate gate. Remaining UI acceptance now depends on the exact protected Preview, counted Run 5, and physical-device/classroom evidence rather than another visual pass.
...
4. Resume at the first unchecked item in section 7. Current counters are 4/5 counted runs, 2/3 completed visual passes, and 4/6 rollout emails. Counted Run 4, its 17-page report, and Email 4 are complete. Do not rerun or recount Runs 1-4, do not accept calibrations A-C as final evidence, and do not resend Emails 1-4. Calibration D is the fresh local Visual Pass 3 candidate evidence; commit/push its explicit intended files, record the exact SHA, and continue with exact protected Preview verification and Run 5. Use read → plan → implement → test.
