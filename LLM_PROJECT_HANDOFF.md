...
1. Commit and push evidence harness SHA `1e0b68b4b93eb557588b1184b122f8800c82653e` plus this handoff checkpoint using only explicit intended files; keep `AGENTS.md` and `KEYUR_WORKFLOW.md` unstaged. Wait for the resulting Ready protected Preview from the exact checkpoint HEAD; do not promote Production.
2. Reconfirm the private live Sheet baseline of two existing `RawSubmissions` test rows and zero `BestResults` rows.
3. From a fresh browser context, run counted Run 5 against the exact protected Preview at 1024 x 768, Touch Only, standard settings. Verify the production-shaped client field is exactly boolean false, the Preview proxy stores `IsTest=TRUE`, one accepted matching receipt, exactly three total `RawSubmissions` test rows, zero `BestResults` rows, and duplicate idempotency without a fourth raw row.
...
4. Resume at the first unchecked item in section 7. Current counters are 4/5 counted runs, 2/3 completed visual passes, and 4/6 rollout emails. Counted Run 4, its 17-page report, and Email 4 are complete. Do not rerun or recount Runs 1-4, do not accept calibrations A-I or superseded Preview attempts as final evidence, and do not resend Emails 1-4. The exact protected-Preview accessibility gate passed at SHA `4b1678d583e90274989e691235efd81ec371fbb4`; the corrected live evidence harness is `1e0b68b4b93eb557588b1184b122f8800c82653e`; and the private Sheet baseline is two raw test rows and zero best rows. Commit this handoff checkpoint, wait for its exact Ready Preview, then run counted Run 5 from a fresh browser context. Use read → plan → implement → test.
