...
- Visual Pass 3 UI candidate commit: `7257ad13abe194fcf2a875fbb9b2bd5fbbbf7bee` (`feat: complete visual pass 3 candidate`). Final protected-Preview evidence-harness commit: `6071fee98aca626b9ec46641782bed6da2d88609` (`test: prove preview submission idempotency`), which also contains the unchanged verified UI candidate. Use `6071fee98aca626b9ec46641782bed6da2d88609` as the evidence SHA for counted Run 5. A later handoff-only checkpoint may be the Preview's tested app SHA without changing this evidence SHA.
...
- Last fully calibrated committed evidence base: `6071fee98aca626b9ec46641782bed6da2d88609`. It contains the verified UI, protected-Preview cookie bootstrap, committed app/evidence/config freshness guards, transient WebGL context-loss listener, and the live-only exact-payload idempotency probe. A final failure-output privacy correction is pending commit and fresh calibration, so do not use `6071fee98aca626b9ec46641782bed6da2d88609` for counted Run 5.
...
1. Commit this final Calibration F reporting checkpoint and push committed evidence SHA `6071fee98aca626b9ec46641782bed6da2d88609` plus the handoff-only checkpoint using only explicit intended files. Wait for green GitHub CI and a successful protected Vercel Preview built from the exact pushed HEAD; do not promote Production.
...
4. Resume at the first unchecked item in section 7. Current counters are 4/5 counted runs, 2/3 completed visual passes, and 4/6 rollout emails. Counted Run 4, its 17-page report, and Email 4 are complete. Do not rerun or recount Runs 1-4, do not accept calibrations A-F as final evidence, and do not resend Emails 1-4. Commit the final failure-output privacy correction, run one fresh full local calibration from that exact SHA, update this handoff with the new evidence SHA, then continue with exact protected Preview verification and Run 5. Use read → plan → implement → test.
