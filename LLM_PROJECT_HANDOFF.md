...
- Visual Pass 3 UI candidate commit: `7257ad13abe194fcf2a875fbb9b2bd5fbbbf7bee` (`feat: complete visual pass 3 candidate`). Final protected-Preview evidence-harness commit: `4b1678d583e90274989e691235efd81ec371fbb4` (`test: stabilize vacuole inspection route`), which contains the unchanged verified UI candidate plus the complete privacy, freshness, idempotency, Preview-only safety, accurate target-labeling, and bounded visible-control navigation sequence. Use `4b1678d583e90274989e691235efd81ec371fbb4` as the evidence SHA for counted Run 5. A later handoff-only checkpoint may be the Preview's tested app SHA without changing this evidence SHA.
...
1. Commit and push this protected-Preview accessibility checkpoint using only `LLM_PROJECT_HANDOFF.md`; keep `AGENTS.md` and `KEYUR_WORKFLOW.md` unstaged. Wait for the resulting Ready protected Preview from the exact checkpoint HEAD; do not promote Production.
2. Reconfirm the private live Sheet baseline of one existing `RawSubmissions` test row and zero `BestResults` rows.
3. Run counted Run 5 against the exact protected Preview at 1024 x 768, Touch Only, standard settings, using the real synthetic backend path with `isTest=true`. Verify one accepted receipt, exactly two total `RawSubmissions` test rows, zero `BestResults` rows, and duplicate idempotency without another raw row.
...
4. Resume at the first unchecked item in section 7. Current counters are 4/5 counted runs, 2/3 completed visual passes, and 4/6 rollout emails. Counted Run 4, its 17-page report, and Email 4 are complete. Do not rerun or recount Runs 1-4, do not accept calibrations A-I or superseded Preview attempts as final evidence, and do not resend Emails 1-4. The exact protected-Preview accessibility gate passed at app/evidence SHA `4b1678d583e90274989e691235efd81ec371fbb4`; the private Sheet baseline is one raw test row and zero best rows. Commit this handoff checkpoint, wait for its exact Ready Preview, then run counted Run 5. Use read → plan → implement → test.
