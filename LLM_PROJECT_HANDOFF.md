...
1. Commit and push the final-audit source, corrected release checklist, and this verified posture without staging the user-owned instruction files.
2. Regenerate the 20-page final audit PDF against that exact audit commit, render it back to PNG, inspect every page, verify appended Run 5 pages remain pixel-identical, and record the final report hash.
3. Commit and push the verified final-report checkpoint, then send Email 6 exactly once with that PDF attached. If delivery is ambiguous, inspect Sent Mail before any retry.
4. Record the successful delivery in this handoff, commit/push the closeout checkpoint, and verify the branch matches upstream with only the user-owned instruction-file edits remaining.
...
4. Resume at the first unchecked item in section 7. Current counters are 5/5 counted runs, 3/3 completed visual passes, and 5/6 rollout emails. The final swarm audit is GO; only exact-commit final-report verification, Email 6, and delivery closeout remain. Do not rerun or recount Runs 1-5, do not accept calibrations or superseded Preview attempts as counted evidence, and do not resend Emails 1-5. Counted Run 5 passed at app SHA `f64fb5301d16f2275d4fc35297d1e4ed2ed3d9f5` with evidence SHA `1e0b68b4b93eb557588b1184b122f8800c82653e`; the private Sheet has three raw test rows, no duplicate fourth row, and zero best rows; and Email 5 was sent with the verified 19-page PDF. Use read → plan → implement → test.
